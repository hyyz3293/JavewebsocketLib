...
import com.jack.websocket.dispatcher.ResponseProcessEngine;
...
import com.jack.websocket.request.RequestFactory;
...
import com.jack.websocket.response.ResponseFactory;
import com.jack.websocket.util.LogUtil;
import com.jack.websocket.util.SocketUtils;
...
public class WebSocketManager {
    ...
    private ReconnectManager getDefaultReconnectManager() {
        return new DefaultReconnectManager(this, mSetting, new ReconnectManager.OnConnectListener() {
            @Override
            ...
            @Override
            public void onDisconnectCount(int count) {
                LogUtil.i(TAG, "重连失败次數" + count);
                mSetting.getResponseDispatcher().onReConnectCount(count, mDelivery);
            }
        });
    }
    ...
    public void login() {
        ...
        if (mSetting.isSetupKeepAlive()) {
            //启动保活服务
            KeepLive.startWork(mSetting.getApplication(), KeepLive.RunMode.ENERGY, mSetting.getForegroundNotification(),
    ...
    public void logout() {
        try {
            send(SocketUtils.getSendMessage("C_Logout"));
            Thread.sleep(200);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void heartbeat() {
        mHandler.postDelayed(heartBeatRunnable, mSetting.getConnectionLostTimeout() * 1000);//开启心跳检测
    }
    ...
    private Runnable heartBeatRunnable = new Runnable() {
        ...
        public void run() {
            try {
                send(SocketUtils.getSendMessage("C_Heart"));
            }catch (Exception e) {
                e.printStackTrace();
            }
            mHandler.postDelayed(this, mSetting.getConnectionLostTimeout() * 1000);
        }
    };
