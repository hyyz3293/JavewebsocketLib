...
import com.jack.websocket.response.ErrorResponse;
...
import static com.jack.websocket.dispatcher.MainThreadResponseDelivery.RUNNABLE_TYPE.BYTE_BUFFER_MSG;
import static com.jack.websocket.dispatcher.MainThreadResponseDelivery.RUNNABLE_TYPE.CONNECTED;
import static com.jack.websocket.dispatcher.MainThreadResponseDelivery.RUNNABLE_TYPE.CONNECT_FAILED;
import static com.jack.websocket.dispatcher.MainThreadResponseDelivery.RUNNABLE_TYPE.DISCONNECT;
import static com.jack.websocket.dispatcher.MainThreadResponseDelivery.RUNNABLE_TYPE.NON;
import static com.jack.websocket.dispatcher.MainThreadResponseDelivery.RUNNABLE_TYPE.PING;
import static com.jack.websocket.dispatcher.MainThreadResponseDelivery.RUNNABLE_TYPE.PONG;
import static com.jack.websocket.dispatcher.MainThreadResponseDelivery.RUNNABLE_TYPE.RE_COUNNT;
import static com.jack.websocket.dispatcher.MainThreadResponseDelivery.RUNNABLE_TYPE.SEND_ERROR;
import static com.jack.websocket.dispatcher.MainThreadResponseDelivery.RUNNABLE_TYPE.STRING_MSG;
import static com.jack.websocket.util.ThreadUtil.checkMainThread;
import static com.jack.websocket.util.ThreadUtil.runOnMainThread;
...
public class MainThreadResponseDelivery implements ResponseDelivery {
    ...
    @Override
    public void onReConnectCount(int count) {
        if (isEmpty() || count == 0) {
            return;
        }
        if (checkMainThread()) {
            synchronized (LISTENER_BLOCK) {
                for (SocketListener listener : mSocketListenerList) {
                    listener.onReConnectCount(count);
                }
            }
        } else {
            CallbackRunnable callbackRunnable = getRunnable();
            callbackRunnable.type = RE_COUNNT;
            callbackRunnable.count = count;
            callbackRunnable.mSocketListenerList = mSocketListenerList;
            runOnMainThread(callbackRunnable);
        }
    }
    ...
    private static class CallbackRunnable<T> implements Runnable {
        ...
        public void run() {
            try {
                ...
                if (type == RE_COUNNT && count == null) return;
                synchronized (LISTENER_BLOCK) {
                    switch (type) {
                        case CONNECTED:
                            for (SocketListener listener : mSocketListenerList) {
